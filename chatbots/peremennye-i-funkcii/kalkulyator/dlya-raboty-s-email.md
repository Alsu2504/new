...
{% tabs %}
{% tab title="Описание" %}
**confirm\_email\_subscription(email, sender\_name, bot\_email, callback,client\_name, send\_from\_bot)**

{% hint style="info" %}
Функция понадобится для того, чтобы собирать согласия с клиентов для направления рассылок.&#x20;

Если клиент в мессенджере указал почту, то будет отправляться согласие на рассылку, только после согласия будет создан email-клиент
...
**send\_from\_bot** - отправлять письмо со сбором согласия от имени бота, к которому будет привязан клиент.

{% hint style="warning" %}
Важно! \
Если параметр send\_from\_bot включен, но при этом email-бот НЕ зарегистрирован на почтовике Salebot (например, домен зарегистрирован в Эпл или Гугл)  или НЕ авторизован, то письмо будет направлено по старому способу - с почты info@salebot.pro, то есть будто send\_from\_bot и не включали
{% endhint %}
